...
namespace CsharpEvilcar.Prompt.CaseTyps
{
		internal abstract class Base
		{
			...
			public virtual ReturnValue.Typ Execute(ref string[] parameters)
			{
				...
				{
				InputOutput.Print(GetAskForParameters);
					InputOutput.GetInput(out parameters);
				}
		...
		internal class Selection : Base
		{
			...
			public override ReturnValue.Typ Execute(ref string[] parameters)
			{
				...
				if (UserMessages.General.HelpSymbol.Contains(selection))
				{
		...
		internal sealed partial class Main : Selection
		{
			...
			public bool Execute()
			{
				...
				InputOutput.Print(code.Text + string.Join("-", casepath) + "\n\n" + "Help" + "\n" + code.Case.Help + "\n\n" + UserMessages.General.SyntaxHead + "\n" + code.Case.Syntax);
				}
